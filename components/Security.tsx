    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-brand-earth text-sm">Protect your root of trust with air-gapped hardware and recovery planning.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-off-white/40 border border-border rounded-3xl p-8 space-y-8">
             <div className="flex items-center gap-4">
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center border border-border">
                   <Key className="text-accent-earth" size={32} />
                </div>
                <div>
                   <h3 className="text-xl font-bold">Local BIP-39 Vault</h3>
                   <p className="text-sm text-brand-earth">Manage your seed phrases. SatoshiLayer never transmits this data.</p>
                </div>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <button
                  onClick={handleViewMnemonic}
                  className="p-6 bg-white border border-border rounded-2xl text-left hover:border-brand-earth transition-all group"
                >
                   <p className="text-[10px] font-black uppercase text-brand-earth mb-2 group-hover:text-brand-earth">Master Seed</p>
                   <p className="font-mono text-xs text-brand-earth">View Recovery Phrase</p>
                </button>
                <button
                  onClick={handleExportVault}
                  className="p-6 bg-white border border-border rounded-2xl text-left hover:border-brand-earth transition-all group"
                >
                   <p className="text-[10px] font-black uppercase text-brand-earth mb-2 group-hover:text-brand-earth">Vault Export</p>
                   <p className="font-mono text-xs text-brand-earth">Download Encrypted JSON</p>
                </button>
                <button
                  onClick={() => context?.notify('info', 'Verification session starting...')}
                  className="p-6 bg-white border border-border rounded-2xl text-left hover:border-brand-earth transition-all group"
                >
                   <p className="text-[10px] font-black uppercase text-brand-earth mb-2 group-hover:text-brand-earth">Health Check</p>
                   <p className="font-mono text-xs text-brand-earth">Verify Physical Backup</p>
                </button>
             </div>

             {showMnemonic && (
                <div className="mt-8 p-8 bg-white border border-orange-500/30 rounded-[2rem] animate-in zoom-in duration-300 relative">
                   <button onClick={() => { setShowMnemonic(false); setMnemonic(null); }} className="absolute top-4 right-4 text-brand-earth hover:text-white">✕</button>
                   <h4 className="text-xs font-black uppercase text-accent-earth mb-6 tracking-widest">Your Recovery Phrase</h4>
                   <div className="grid grid-cols-3 gap-4">
                      {mnemonic?.split(' ').map((word, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <span className="text-[9px] text-brand-earth font-mono">{i+1}.</span>
                          <span className="text-sm font-bold text-brand-deep">{word}</span>
                        </div>
                      ))}
                   </div>
                </div>
             ...
             {showPinPrompt && (
                <div className="fixed inset-0 z-[300] bg-brand-deep/80 backdrop-blur-sm flex items-center justify-center p-4">
                   <div className="bg-off-white border border-border rounded-[2.5rem] p-8 w-full max-w-sm space-y-6 shadow-2xl">
                      <div className="text-center space-y-2">
                        <Lock className="text-accent-earth mx-auto" size={32} />
                        <h3 className="font-black italic uppercase text-lg">Identity Challenge</h3>
                        <p className="text-xs text-brand-earth">Enter your Enclave PIN to reveal secrets.</p>
                      </div>
                      <input
                        type="password"
                        autoFocus
                        ...
                        autoCapitalize="off"
                        spellCheck="false"
                        className="w-full bg-white border border-border rounded-2xl py-4 text-center text-2xl font-mono tracking-widest focus:outline-none focus:border-orange-500"
                        maxLength={8}
                        placeholder="••••••••"
                        aria-label="Enter PIN"
                      />
                      <div className="flex gap-4">
                        <button onClick={() => { setShowPinPrompt(false); setPinEntry(''); }} className="flex-1 py-4 bg-border rounded-2xl font-black text-xs uppercase">Cancel</button>
                        <button
                          onClick={confirmPinAndDecrypt}
                          disabled={isDecrypting || pinEntry.length < 4}
                          className="flex-1 py-4 bg-accent-earth rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2"
                        >
        ...
        <div className="space-y-6">
          <div className="bg-off-white/40 border border-border rounded-3xl p-8 space-y-6">
             ...
             <button className="w-full py-3 bg-border hover:bg-brand-earth rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2">
          ...
          <div className="bg-white border border-border rounded-3xl p-8">
             <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="font-bold text-sm text-brand-deep">Security Audit</h4>
             </div>
             <p className="text-xs text-brand-earth leading-relaxed mb-4">
