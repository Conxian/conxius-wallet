...
const BackupAuditModal: React.FC<BackupAuditModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-brand-deep/80 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="w-full max-w-md bg-white border border-border rounded-[3rem] p-8 space-y-6 relative shadow-2xl">
        <button onClick={handleClose} className="absolute top-6 right-6 text-brand-earth hover:text-white transition-colors">
          <X size={24} />
        </button>

        <div className="text-center space-y-2">
          <div className="w-16 h-16 bg-accent-earth/10 rounded-2xl flex items-center justify-center mx-auto text-accent-earth mb-2">
            <ShieldCheck size={32} />
          </div>
          <h3 className="text-xl font-black italic uppercase tracking-tighter text-white">Backup Health Audit</h3>
          <p className="text-xs text-brand-earth uppercase tracking-widest font-bold">Milestone M8 • Protocol Alpha</p>
        </div>

        {step === 'pin' && (
          <form onSubmit={handlePinSubmit} className="space-y-6 animate-in slide-in-from-bottom-4">
            <p className="text-xs text-brand-earth text-center px-4">Enter your Enclave PIN to authorize the audit.</p>
            <input
              type="password"
              value={pin}
              ...
              autoCapitalize="off"
              spellCheck="false"
              className="w-full bg-off-white border border-border rounded-2xl py-4 text-center text-2xl font-mono text-white tracking-[1em] focus:outline-none focus:border-orange-500/50"
            />
            {error && <p className="text-[10px] text-red-500 text-center font-bold uppercase">{error}</p>}
            <button
              disabled={isLoading || pin.length < 4}
              className="w-full bg-white text-black font-black py-4 rounded-2xl uppercase tracking-widest hover:bg-off-white transition-all disabled:opacity-50"
            >
        ...
        {step === 'mnemonic' && (
          <form onSubmit={handleMnemonicSubmit} className="space-y-6 animate-in slide-in-from-right-4">
            <div className="bg-orange-500/10 border border-orange-500/20 p-4 rounded-2xl flex gap-3">
              <AlertTriangle size={18} className="text-accent-earth shrink-0" />
            ...
            <textarea
              ...
              autoCapitalize="off"
              spellCheck="false"
              className="w-full h-32 bg-off-white border border-border rounded-2xl p-4 text-sm font-mono text-brand-deep focus:outline-none focus:border-orange-500/50 resize-none"
            />
            {error && <p className="text-[10px] text-red-500 text-center font-bold uppercase">{error}</p>}
            <button
              disabled={isLoading || !mnemonicInput.trim()}
              className="w-full bg-accent-earth text-white font-black py-4 rounded-2xl uppercase tracking-widest hover:bg-accent-earth/90 transition-all disabled:opacity-50"
            >
        ...
        {step === 'success' && (
          <div className="text-center space-y-6 animate-in zoom-in-95">
            <div className="bg-green-500/10 border border-green-500/20 p-6 rounded-[2rem] space-y-2">
              ...
              <p className="text-xs text-brand-earth">Your physical backup is healthy and matches the enclave's root.</p>
            </div>
            <button
              onClick={handleClose}
              className="w-full bg-white text-black font-black py-4 rounded-2xl uppercase tracking-widest hover:bg-white transition-all"
            >
              Done
            </button>
          </div>
        ...
        {step === 'failure' && (
          <div className="text-center space-y-6 animate-in zoom-in-95">
            <div className="bg-red-500/10 border border-red-500/20 p-6 rounded-[2rem] space-y-2">
              ...
              <p className="text-xs text-brand-earth">The phrase entered does not match your active vault.</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => setStep('mnemonic')}
                className="flex-1 bg-border text-white font-black py-4 rounded-2xl uppercase tracking-widest hover:bg-brand-earth transition-all flex items-center justify-center gap-2"
              >
                <RefreshCcw size={16} /> Retry
              </button>
              <button
                onClick={handleClose}
                className="flex-1 bg-white text-black font-black py-4 rounded-2xl uppercase tracking-widest hover:bg-off-white transition-all"
              >
                Abort
              </button>
            </div>
