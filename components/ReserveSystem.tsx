      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black tracking-tighter text-brand-deep flex items-center gap-3">
            <Landmark className="text-accent-earth" />
            Sovereign Reserve Engine
          </h2>
          <p className="text-brand-earth text-sm italic">Verifiable Proof of Reserves (PoR) for all wrapped and bridged assets.</p>
        </div>
        <div className="flex gap-4">
           <div className="bg-off-white border border-border px-6 py-3 rounded-2xl flex items-center gap-4">
              ...
              <div>
                 <p className="text-[10px] font-black uppercase text-brand-earth">Aggregate Reserve Ratio</p>
                 <div className="flex items-center gap-2">
                    <p className="text-xl font-bold text-green-500 font-mono">{reserveRatio}%</p>
                    <span className="text-[10px] text-brand-earth font-bold uppercase">(Over-collateralized)</span>
                 </div>
              </div>
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 space-y-10">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {reserves.map((res, i) => (
                 <div key={i} className="bg-off-white/40 border border-border rounded-[2.5rem] p-8 space-y-6 group hover:border-orange-500/30 transition-all">
                    <div className="flex justify-between items-start">
                       <div>
                          <p className="text-[10px] font-black uppercase text-brand-earth tracking-widest">{res.asset} Backing</p>
                          <h4 className="text-2xl font-black text-brand-deep mt-1">{res.collateralRatio}%</h4>
                       </div>
                    ...
                    <button className="w-full py-3 bg-white border border-border rounded-xl text-[10px] font-black uppercase text-brand-earth hover:text-accent-earth hover:border-orange-500/30 transition-all flex items-center justify-center gap-2">
           ...
           <div className="bg-ivory border border-border rounded-[3rem] p-1 shadow-2xl overflow-hidden group">
              <div className="bg-white rounded-[2.8rem] flex flex-col min-h-[400px]">
                 <div className="p-8 border-b border-border bg-off-white/20 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                       <Activity size={18} className="text-accent-earth" />
                       <h3 className="text-xs font-black uppercase tracking-widest text-brand-earth font-mono">LIVE_COLLATERAL_MONITOR</h3>
                    </div>
                    <div className="flex items-center gap-4">
                       <span className="text-[10px] text-brand-earth font-mono uppercase italic">Last Global Audit: {lastAudit}</span>
                       <button onClick={handleVerify} disabled={isVerifying} className="text-accent-earth hover:text-orange-400 transition-all flex items-center gap-2 bg-orange-500/10 px-4 py-2 rounded-xl border border-orange-500/20 text-[10px] font-black uppercase">
                 ...
                 <div className="flex-1 p-8 font-mono text-[11px] text-brand-earth leading-relaxed overflow-y-auto custom-scrollbar">
                    <p className="text-brand-earth">[SYSTEM] PULLING MERKLE ROOT FROM BITCOIN BLOCK #873,042...</p>
                    ...
                    <p className="text-brand-earth mt-4 border-t border-border pt-4">[SYSTEM] GENERATING ATTESTATION CERTIFICATE #42-AF-93...</p>
                 </div>
              </div>
           </div>
        </div>
                 ...
                 {chartData.map((d, i) => (
                    <div key={i} className="flex justify-between items-center text-[10px] font-bold">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-brand-earth">{d.name}</span>
                       </div>
                       <span className="text-brand-deep">{d.value}%</span>
                    </div>
                 ))}
              </div>
           </div>
                 ...
                 {[
                   ...
                   { date: 'Oct 19', event: 'Rootstock Audit', delta: 'OK' },
                    ...
                    <div key={i} className="flex justify-between items-center border-b border-border pb-3 last:border-0">
                       <div>
                          <p className="text-[10px] font-black uppercase text-brand-earth">{h.date}</p>
                          <p className="text-xs font-bold text-brand-deep">{h.event}</p>
                       </div>
