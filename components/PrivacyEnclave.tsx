      ...
      <div className="space-y-4">
        <div className="flex items-center gap-4">
           ...
           <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-earth">Zero-Knowledge Sovereignty Management</p>
           </div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-gradient-to-br from-off-white/80 to-ivory/80 border border-border rounded-[2.5rem] p-8 relative overflow-hidden">
           ...
           <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
              <div className="space-y-4">
                 ...
                 <p className="text-xs text-brand-deep leading-relaxed font-medium">
                 ...
                 <div className="flex items-center gap-4 pt-2">
                    <button 
                      ...
                      className={`relative w-14 h-7 rounded-full transition-colors ${isDataUnionActive ? 'bg-purple-600' : 'bg-border'}`}
                    >
                       <div className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-all shadow-md ${isDataUnionActive ? 'left-8' : 'left-1'}`} />
                    </button>
                    <span className="text-xs font-bold uppercase tracking-widest text-brand-earth">
                       {isDataUnionActive ? 'Broadcasting Offer' : 'Union Inactive'}
                    </span>
                 </div>
              </div>

              <div className="bg-white/80 rounded-3xl p-6 border border-border space-y-4 backdrop-blur-sm">
                 <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black uppercase text-brand-earth">Incentive Status</span>
                    {isDataUnionActive ? (
                       ...
                       <span className="text-[10px] font-bold text-brand-earth uppercase">Standard</span>
                    )}
                 </div>
                 
                 <div>
                    <label className="text-[10px] font-black uppercase text-brand-earth mb-2 block">Min. Ask Price (Negotiable)</label>
                    <div className="flex items-center gap-4">
                       <input 
                          type="range" 
                          min="10" 
                       ...
                       />
                       <span className="text-lg font-mono font-bold text-white">{askPrice} <span className="text-xs text-brand-earth">SATS</span></span>
                    </div>
                 </div>
                 
                 <p className="text-[9px] text-brand-earth italic border-t border-border pt-3">
        ...
        <div className="space-y-6">
          <div className="bg-off-white/40 border border-border rounded-3xl p-8 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <p className="font-bold text-brand-deep">Local-Only Key Generation</p>
                  <p className="text-xs text-brand-earth max-w-[240px]">Keys are generated in the browser's secure enclave and never touch our servers.</p>
                </div>
                <button 
                  onClick={() => setLocalOnly(!localOnly)}
                  className={`relative w-12 h-6 rounded-full transition-colors ${localOnly ? 'bg-accent-earth' : 'bg-border'}`}
                >
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <p className="font-bold text-brand-deep">Nostr Metadata Storage</p>
                  <p className="text-xs text-brand-earth max-w-[240px]">Store your wallet profile and D.i.D info on decentralized Nostr relays instead of databases.</p>
                </div>
                <button 
                  onClick={() => setNostrMetadata(!nostrMetadata)}
                  className={`relative w-12 h-6 rounded-full transition-colors ${nostrMetadata ? 'bg-accent-earth' : 'bg-border'}`}
                >
          ...
          <div className="bg-off-white/40 border border-border rounded-3xl p-8 space-y-8">
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <p className="font-bold text-brand-deep">Always Route through Tor</p>
                  <p className="text-xs text-brand-earth max-w-[240px]">Obfuscate your IP address for all RPC and API calls.</p>
                </div>
                <button 
                  onClick={() => {
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors ${torEnabled ? 'bg-accent-earth' : 'bg-border'}`}
                >
                  <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${torEnabled ? 'left-7' : 'left-1'}`} />
                </button>
              </div>

              <div className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-border text-xs text-brand-earth">
        ...
        <div className="space-y-6">
          <div className="bg-orange-500/5 border border-orange-500/10 rounded-3xl p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h4 className="font-bold text-xs uppercase tracking-widest text-accent-earth">Satoshi AI: Privacy Audit</h4>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-brand-deep">Privacy Score</span>
                <span className={`text-lg font-bold ${privacyResult.score > 80 ? 'text-green-500' : 'text-accent-earth'}`}>{privacyResult.score}/100</span>
              </div>
              ...
              <div className="space-y-2">
                {privacyResult.recommendations.length > 0 ? (
                  privacyResult.recommendations.map((rec, i) => (
                    <p key={i} className="text-[10px] text-brand-earth leading-relaxed flex items-start gap-2">
                      <Info size={10} className="mt-0.5 text-accent-earth shrink-0" />
                      {rec}
                    </p>
                  ))
                ) : (
          ...
          <div className="bg-off-white/20 border border-border rounded-3xl p-8 space-y-4">
            <h4 className="font-bold text-sm text-brand-deep flex items-center gap-2">
            ...
            <button className="w-full py-2 bg-border rounded-xl text-[10px] font-bold text-brand-earth uppercase tracking-widest hover:bg-brand-earth transition-colors">
