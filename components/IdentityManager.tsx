  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-10 animate-in fade-in duration-500 pb-24">
      <header className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-black italic uppercase tracking-tighter text-brand-deep">Identity Enclave</h2>
          <p className="text-brand-earth text-sm italic">Verification through Proof-of-Work and Web5 Decentralization.</p>
        </div>
        <div className="flex gap-4">
            <button
                onClick={handleConnectWeb5}
                disabled={dwnStatus === 'connected' || dwnStatus === 'connecting'}
                className="bg-border hover:bg-brand-earth text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95"
            >
                {dwnStatus === 'connecting' ? <Loader2 size={18} className="animate-spin" /> : <Globe size={18} />}
                {dwnStatus === 'connected' ? 'Web5 Active' : 'Initialize Web5'}
            </button>
            <button onClick={() => setIsModalOpen(true)} className="bg-accent-earth hover:bg-accent-earth/90 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 transition-all shadow-xl shadow-orange-600/20 active:scale-95">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-off-white/40 border border-border rounded-[3.5rem] p-1 shadow-2xl overflow-hidden group">
             <div className="bg-white rounded-[3.4rem] p-10 space-y-10 relative overflow-hidden text-center">
                <div className="w-24 h-24 rounded-[2rem] border-4 border-orange-500/20 p-1 bg-off-white shadow-2xl mx-auto relative">
                   ...
                   {dwnStatus === 'connected' && (
                       <div className="absolute -bottom-2 -right-2 bg-blue-600 p-1.5 rounded-full border-4 border-ivory">
                           <Cloud size={12} className="text-white" />
                       </div>
                   )}
                </div>
                <div>
                   <h3 className="text-2xl font-black text-brand-deep flex items-center justify-center gap-2">
                      {profile.alias} <ShieldCheck size={20} className="text-blue-500" />
                   </h3>
                   <p className="text-[10px] font-black uppercase tracking-[0.3em] text-accent-earth mt-2">Verified Peer Root</p>
                </div>
                <div className="space-y-4">
                    <div className="bg-off-white border border-border rounded-2xl p-4 flex items-center justify-between text-left">
                       <div>
                          <p className="text-[9px] font-black text-brand-earth uppercase">Primary BTC DID</p>
                          <p className="text-[10px] font-mono text-brand-earth truncate w-32">{profile.did}</p>
                       </div>
                       <button onClick={() => navigator.clipboard.writeText(profile.did)} className="p-2 hover:bg-border rounded-lg text-brand-earth">
                          <Copy size={14} />
                       </button>
                    </div>
           ...
           {dwnStatus === 'connected' && (
               <div className="bg-off-white/40 border border-border rounded-[2.5rem] p-8 space-y-4">
                   ...
                   <div className="space-y-2">
                       <div className="flex justify-between text-[10px]">
                           <span className="text-brand-earth uppercase font-black">Storage Mode</span>
                           <span className="text-brand-deep font-bold">Decentralized</span>
                       </div>
                       <div className="flex justify-between text-[10px]">
                           <span className="text-brand-earth uppercase font-black">Sync Status</span>
        ...
        <div className="lg:col-span-8 space-y-10">
           ...
           <div className="bg-purple-600/10 border border-purple-500/20 rounded-[3rem] p-10 space-y-8 relative overflow-hidden group">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                 <div className="space-y-3">
                    ...
                    <p className="text-xs text-brand-earth max-w-md leading-relaxed italic">Enable encrypted multi-sig coordination and decentralized profile metadata via relays.</p>
                 </div>
                 {nostrKeys ? (
              ...
              {nostrKeys && (
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-4">
                    <div className="p-6 bg-white border border-border rounded-3xl space-y-2">
                       <p className="text-[9px] font-black uppercase text-brand-earth">Public Key (npub)</p>
                       <p className="text-[10px] font-mono text-brand-deep break-all bg-off-white/50 p-3 rounded-xl border border-border">{nostrKeys.npub}</p>
                    </div>
                    <div className="p-6 bg-white border border-border rounded-3xl space-y-2">
                       <p className="text-[9px] font-black uppercase text-brand-earth">Private Enclave Key (nsec)</p>
                       <p className="text-[10px] font-mono text-brand-earth blur-md hover:blur-0 cursor-pointer break-all bg-off-white/50 p-3 rounded-xl border border-border transition-all">{nostrKeys.nsec}</p>
                    </div>
                 </div>
              )}
           </div>

           <div className="bg-off-white/20 border border-border rounded-[3rem] p-10 space-y-6">
              <div className="flex items-center gap-3">
                 <div className="bg-blue-500/20 p-2 rounded-xl">
                    <Zap size={20} className="text-blue-500" />
                 </div>
                 <h3 className="text-xs font-black uppercase tracking-widest text-brand-earth">Identity Audit</h3>
              </div>
              <div className="bg-white p-8 rounded-[2rem] border border-border relative">
                 <div className="absolute top-0 right-0 p-8 opacity-[0.02]"><Bot size={120} /></div>
                 {isLoadingInsight ? (
                    <div className="flex flex-col items-center justify-center py-10 gap-4 text-brand-earth">
                       <Loader2 className="animate-spin" size={32} />
                       <p className="text-[10px] font-black uppercase tracking-widest animate-pulse">Satoshi AI auditing DID path...</p>
                    </div>
                 ) : (
                    <p className="text-xs font-mono text-brand-earth leading-relaxed italic whitespace-pre-wrap">{insight}</p>
                 )}
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="bg-accent-earth/10 border border-orange-500/20 rounded-[2.5rem] p-8 space-y-4">
                   ...
                   <p className="text-[10px] text-brand-earth leading-relaxed">
               ...
               <button onClick={handleLnLogin} className="bg-yellow-500/10 border border-yellow-500/20 rounded-[2.5rem] p-8 space-y-4 text-left hover:bg-yellow-500/20 transition-all group">
                   ...
                   <p className="text-[10px] text-brand-earth leading-relaxed group-hover:text-brand-deep">
