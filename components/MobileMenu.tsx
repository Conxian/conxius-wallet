...
import { Fingerprint, LayoutDashboard, CreditCard, Trophy, FlaskConical, Gavel, Landmark, Briefcase, BarChart3, BookOpen, Package, Rocket, Coins, Repeat, UserCheck, Network, Lock, Shield, Settings, X, ChevronRight, LogOut, Zap, Layers, Palette, ShoppingBag, Globe } from 'lucide-react';
...
const MobileMenu: React.FC<MobileMenuProps> = ({ setActiveTab, activeTab }) => {
  ...
  return (
    <div className="p-6 pb-32 animate-in slide-in-from-bottom-10 duration-300">
      ...
      <div className="bg-off-white/50 border border-border p-6 rounded-[2rem] mb-8 flex items-center justify-between shadow-xl">
        <div className="flex items-center gap-4">
           <div className="w-14 h-14 bg-accent-earth rounded-2xl flex items-center justify-center text-white shadow-lg shadow-orange-600/20">
              <Zap size={24} fill="currentColor" />
           </div>
           <div>
              <h3 className="text-lg font-black text-brand-deep italic uppercase tracking-tighter">Sovereign Mode</h3>
              <p className="text-[10px] font-mono text-brand-earth font-bold uppercase">
                 {context?.state.isMainnetLive ? 'Mainnet Active' : 'Testnet Alpha'}
              </p>
           </div>
        </div>
        <div className="text-right">
           <p className="text-[10px] font-black uppercase text-brand-earth">Score</p>
           <p className="text-xl font-bold text-accent-earth">{context?.state.sovereigntyScore}</p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {MENU_SECTIONS.map((section, idx) => (
          <div key={idx}>
            <h4 className="text-xs font-black uppercase tracking-[0.2em] text-brand-earth mb-4 ml-2">{section.title}</h4>
      ...
      <div className="mt-10 p-4 border-t border-border">
         ...
         <p className="text-center text-[9px] text-brand-earth mt-4 font-mono">v1.9.5-STABLE • Build 872941</p>
      </div>
    </div>
  );
};
