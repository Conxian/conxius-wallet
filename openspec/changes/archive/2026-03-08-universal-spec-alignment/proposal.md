...
## Why
The current project documentation (PRDs, Implementation Registry, Roadmap) has version inconsistencies (v1.9.5 vs v1.9.5) and is scattered across multiple locations. To maintain the "Clean Break" native migration integrity, we need a single source of truth using the OpenSpec framework.

## What's Changing
- **Centralization**: All high-level specifications are being moved to `openspec/specs/`.
- **Version Normalization**: Aligning all documents to the target **v1.9.5 (Citadel Native)**.
