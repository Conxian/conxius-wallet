---
title: Documentation Home
layout: page
permalink: /docs/
---

# Conxius Wallet Documentation (v1.9.2)

Welcome to the official technical documentation for the Conxius Wallet, the sovereign-first mobile gateway for the Bitcoin ecosystem.

## 🏗️ Architecture & Strategy
- [Architecture Overview](architecture/)
- [SDK Ownership & Version Policy](architecture/SDK_OWNERSHIP_AND_VERSION_POLICY.md)
- [Wasm-First Gateway Strategy](architecture/proposals/WASM_FIRST_GATEWAY_STRATEGY.md)
- [Bridge & Messaging Trust Tiers](architecture/APPROVED_BRIDGE_AND_MESSAGING_SYSTEMS_BY_TRUST_TIER.md)

## 📋 Business, State & Product
- [Business Overview](business/)
- [State & Implementation](state/)
- [Product Requirements (PRD)](business/PRD.md)
- [Implementation Registry](protocols/IMPLEMENTATION_REGISTRY.md)
- [Strategic Roadmap](operations/ROADMAP.md)

## 🛡️ Operations & Security
- [Operations Overview](operations/)
- [Operating Model](operations/OPERATING_MODEL.md)
- [Production Artifact Contracts](operations/PRODUCTION_ARTIFACT_CONTRACTS.md)
- [SAB Wallet Map](operations/SAB_WALLET_MAP.md)
- [Android Release Prep](operations/ANDROID_RELEASE_PREP.md)

## 🗄️ Archive
- [Archive Overview](archive/)

## 🧪 Research & Reports
- [SDK Research](reports/SDK_RESEARCH.md)
- [FDC3 Interoperability Proposal](reports/FDC3_INTEROPERABILITY_PROPOSAL.md)
- [Native Bridge Research](protocols/NATIVE_BRIDGE_RESEARCH.md)

---
*Sovereignty is a journey. This documentation tracks our path to v1.9.2 and beyond.*
