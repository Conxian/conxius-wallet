...
This slice establishes the wallet-side collection and request-binding boundary
for a future authorization flow. It does **not** qualify StrongBox on real
devices and it does **not** verify Play Integrity tokens.
...
plus server-side KeyMint/Play Integrity certificate and verdict verification.
The repository-side tests cover deterministic policy and binding behavior; they
are not a substitute for that hardware and backend qualification.
