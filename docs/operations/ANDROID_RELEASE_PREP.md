...
### 3.1 Canonical compatibility matrix

Treat the Android toolchain as one compatibility matrix. Changes to Kotlin,
the Compose compiler plugin, KSP, AGP, Gradle, or the SDK/NDK pins must be
reviewed and validated together rather than landed as independent upgrades.

| Component | Canonical value | Authoritative source |
| --- | --- | --- |
| AGP | `9.3.0` | `android/gradle/libs.versions.toml` (`agp`) |
| Kotlin and Compose compiler plugin | `2.4.10` | `android/gradle/libs.versions.toml` (`kotlin` and `compose-compiler`) |
| KSP | `2.3.10` | `android/gradle/libs.versions.toml` (`ksp`) |
| Compose BOM | `2026.06.01` | `android/gradle/libs.versions.toml` (`compose-bom`) |
| Gradle | `9.6.1` | `android/gradle/wrapper/gradle-wrapper.properties` |
| JDK / JVM target | `21` | `.github/workflows/ci.yml`, `.github/workflows/android-release.yml`, and module `build.gradle.kts` files |
| Compile SDK | `36` | `android/app/build.gradle.kts` and Android library module `build.gradle.kts` files |
| Target SDK | `35` | `android/app/build.gradle.kts` |
| Minimum SDK | `26` | `android/app/build.gradle.kts` and Android library module `build.gradle.kts` files |
| NDK | `27.2.12479018` | `.github/workflows/ci.yml` and `.github/workflows/android-release.yml` |

`android/variables.gradle` retains synchronized compile, target, and minimum SDK
values as a legacy Capacitor compatibility declaration. The current native
Gradle build does not apply or consume that file, so it is not an authoritative
source for the active Android build configuration.

`android/app/capacitor.build.gradle` is generated by Capacitor. It is not an
authoritative toolchain source and must not be hand-edited; regenerate it only
through the repository's Capacitor workflow.

This matrix reconciles repository declarations. It does not by itself prove
production release readiness or compatibility across the required physical
device, JNI/ABI, Room migration, lifecycle, signing, and Play distribution
evidence. Those checks remain release gates below and in the linked release
baseline work.
...
### 3.4 Play Integrity client boundary (not a production gate)
...
exceptions both remain **pending** through **2026-10-19**. The `esbuild` finding
