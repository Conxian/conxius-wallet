...
- **Validation:** The issue #444 focused and full Vitest suites, TypeScript 6/7 and compatibility checks, lint, runtime contamination guards, and `git diff --check` passed on the code candidate. Negative source tests cover Ark, RGB, StateChain, Maven, Taproot Assets, Monetization, Wormhole/NTT, Lightning/backends, swaps, DLC, PayJoin/CoinJoin, merchant/protocol fallbacks, seed-based PSBT helpers, raw enclave/Breez/BDK bypasses, and Marketplace preview behavior. The local web build reached Vite chunk rendering and was killed by the constrained 3.8 GiB devbox; Android tasks remain blocked by the missing SDK/`ANDROID_HOME`. Final CI still requires web build, Playwright E2E, Android app unit tests, Android release lint, and relevant module compilation. See the [issue #444 evidence record](../reports/ISSUE_444_VALUE_OPERATION_GATE_CONTAINMENT.md).
