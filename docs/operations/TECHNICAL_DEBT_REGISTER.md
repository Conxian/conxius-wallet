...
- **Validation:** `./gradlew --no-daemon :app:tasks --all` passed after removing the redundant Kotlin plugin, migrating `kotlinOptions` to `compilerOptions`, and applying the required Compose compiler plugin. `pnpm run check:android-toolchain` guards catalog, compile SDK, hosted SDK/NDK, Java, and wrapper matrix coherence. PR #390's Compose scan card is source-integrated, but `./gradlew --no-daemon :app:testDebugUnitTest :core-bitcoin:test` and Android compilation remain blocked by the missing Android SDK.
- **Target milestone:** M16 release baseline
