...
- **Validation:** Protocol-by-protocol evidence review, native unit/instrumentation tests, and negative tests for unsupported production operations. BitVM2 requires arbitrary/malformed/wrong-key/mutated/encoding/tap-index/binding cases, explicit non-authoritative simulation labels, exact TypeScript/Kotlin quarantine-envelope parity, and proof that unsupported/simulated/caller-fabricated results never invoke a signer.
