...
| --- | --- | --- | --- | --- |
| `GHSA-3gc7-fjrx-p6mg` | `bigint-buffer@1.1.5` | High | Time-bound `exception`, approval `pending`, expires **2026-10-19** | Production Wormhole/Solana paths; advisory says `>=1.1.6`, but npm has no `1.1.6+` release. `CON-1525` and GitHub `#399` are tracking context only, not approval evidence. |
| `GHSA-848j-6mx2-7j84` | `elliptic@6.6.1` | Low | Time-bound `exception`, approval `pending`, expires **2026-10-19** | Production Wormhole/CosmJS and Payjoin paths; advisory says `>=6.6.2`, but npm has no `6.6.2` release. `CON-1525` and GitHub `#399` are tracking context only, not approval evidence. |
