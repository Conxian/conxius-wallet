...
**Issue #444 candidate reviewed:** 2026-07-25 at
`bf16fea0cb551bdc5c1147d19af6d42fdd32a97a` on
`charlie/issue-444-value-operation-gate`; this is local branch evidence, not a
`main` promotion or release acceptance.
...
- **Priority:** P0
- **Status:** In Progress — synthetic-success/direct-bypass containment is complete for reviewed issue #444 wallet paths; external trust, replay, provider qualification, receipts/finality, rollout, and unreviewed paths remain open.
- **Affected paths:** `services/value-operation-gate.ts`, `services/value-operation-evidence-verifier.ts`, `services/value-operations.ts`, `services/value-operation-result.ts`, `services/value-operation-authorization-queue.ts`, `services/value-signer.ts`, `services/bitcoin-broadcast.ts`, reviewed App/context/UI and protocol adapters, and `android/core-bitcoin/src/main/kotlin/com/conxius/wallet/bitcoin/BdkManager.kt`.
- **Impact:** Several protocol paths were described as bridged or production-ready while returning stubs, simulations, placeholder identifiers, direct signer/broadcaster calls, or incomplete native outcomes. Issue #444 replaces reviewed value paths with a deterministic canonical envelope, one application gate, exact authorization/artifact binding, a native-only gate-bound PSBT signer, typed outcomes, and source/runtime regressions that prevent synthetic txids, preimages, booleans, or local completion flags from satisfying submitted/settled contracts. BitVM2 remains separately quarantined under issue #427.
- **Owner:** Unassigned
...
- **Validation:** The issue #444 focused and full Vitest suites, TypeScript 6/7 and compatibility checks, lint, runtime contamination guards, and `git diff --check` passed on the code candidate. Negative source tests cover Ark, RGB, StateChain, Maven, Taproot Assets, Monetization, Wormhole/NTT, Lightning/backends, swaps, DLC, PayJoin/CoinJoin, merchant/protocol fallbacks, seed-based PSBT helpers, raw enclave/Breez/BDK bypasses, and Marketplace preview behavior. The isolated local web production build completed after transforming 4,594 modules, and the full local Playwright suite passed all 36 tests across Chromium and mobile Chrome. These are local results, not hosted CI claims. Android tasks remain blocked by the missing SDK/`ANDROID_HOME` and require hosted CI, including app unit tests, release lint, and relevant module compilation. See the [issue #444 evidence record](../reports/ISSUE_444_VALUE_OPERATION_GATE_CONTAINMENT.md).
...
- **Metrics baseline:** Reviewed in-repo production callers no longer use the old bare execution shapes. The concrete production evidence verifier always returns `unsupported_provider`, so containment is implemented but no production value operation is authorized. Process-local stage tracking is not durable replay prevention.
- **Evidence:** [`ISSUE_444_VALUE_OPERATION_GATE_CONTAINMENT.md`](../reports/ISSUE_444_VALUE_OPERATION_GATE_CONTAINMENT.md); [issue #444](https://github.com/Conxian/conxius-wallet/issues/444); [`GAP_MATRIX_2026.md`](../reports/GAP_MATRIX_2026.md); [`v1.9.5_CODE_GAP_MAPPING.md`](../reports/v1.9.5_CODE_GAP_MAPPING.md); [`GAPS_AND_RECOMMENDATIONS_ROUND_2.md`](../archive/GAPS_AND_RECOMMENDATIONS_ROUND_2.md)
...
- **Priority:** P0
- **Status:** In Progress — client collection/request binding and wallet-local fail-closed containment are implemented; real-device/provider qualification, authoritative backend verification, durable freshness/replay, provider receipts/finality, rollout controls, and independent review remain open.
- **Affected paths:** `android/core-crypto/src/main/kotlin/com/conxius/wallet/crypto/KeySecurityPolicy.kt`, `android/core-crypto/src/main/kotlin/com/conxius/wallet/crypto/KeyMintAuthorizationManager.kt`, `android/core-crypto/src/main/kotlin/com/conxius/wallet/crypto/AuthorizationRequest.kt`, `android/app/src/main/kotlin/com/conxius/wallet/PlayIntegrityPlugin.kt`, the issue #444 value-operation surfaces, `docs/reports/CON_1544_KEYMINT_AUTHORIZATION_BOUNDARY.md`
...
- **Validation:** PR #441 merged the KeyMint policy/evidence/canonical-binding client slice; PR #442 merged Play Integrity SDK `1.6.0` opaque-token acquisition. Issue #444 contains reviewed wallet execution paths, but the concrete verifier always returns `unsupported_provider` and does not consume KeyMint/Play evidence as an authorization decision. Local Android Gradle tests remain SDK-blocked in this devbox. No client/unit/hosted CI result is accepted as hardware qualification, backend verification, distributed replay, or provider finality.
- **Target milestone:** M16 release baseline gate
- **Metrics baseline:** Client evidence boundary and wallet-local containment are present. Backend verifier, real-device evidence matrix, roots/collateral/revocation, trusted time, durable distributed replay, provider receipts/finality, privacy-minimized telemetry, staged rollout/runbook, and independent acceptance are not delivered by issue #444. CON-1512/CON-1517 and CON-1543 are not closed by this wallet change.
- **Evidence:** [`CON_1544_KEYMINT_AUTHORIZATION_BOUNDARY.md`](../reports/CON_1544_KEYMINT_AUTHORIZATION_BOUNDARY.md); [`ISSUE_444_VALUE_OPERATION_GATE_CONTAINMENT.md`](../reports/ISSUE_444_VALUE_OPERATION_GATE_CONTAINMENT.md); [issue #444](https://github.com/Conxian/conxius-wallet/issues/444); [PR #441](https://github.com/Conxian/conxius-wallet/pull/441); [PR #442](https://github.com/Conxian/conxius-wallet/pull/442); [CON-1543](https://linear.app/conxian-labs/issue/CON-1543/p0-operationalize-attestation-roots-collateral-revocation-and); [CON-1512](https://linear.app/conxian-labs/issue/CON-1512/p0-enforce-hardware-backed-signing-and-mandatory-attestation-for-value); [CON-1546](https://linear.app/conxian-labs/issue/CON-1546/p0-add-centralized-wallet-value-operation-gate-and-quarantine); [CON-1519](https://linear.app/conxian-labs/issue/CON-1519/p0-complete-independent-security-review-and-release-acceptance)
...
| --- | --- | --- | --- | --- |
| `GHSA-3gc7-fjrx-p6mg` | `bigint-buffer@1.1.5` | High | Time-bound `exception`, approval `pending`, expires **2026-09-19** | Production Wormhole/Solana paths; advisory says `>=1.1.6`, but npm has no `1.1.6+` release. `CON-1525` and GitHub `#399` are tracking context only, not approval evidence. |
| `GHSA-848j-6mx2-7j84` | `elliptic@6.6.1` | Low | Time-bound `exception`, approval `pending`, expires **2026-09-19** | Production Wormhole/CosmJS and Payjoin paths; advisory says `>=6.6.2`, but npm has no `6.6.2` release. `CON-1525` and GitHub `#399` are tracking context only, not approval evidence. |
