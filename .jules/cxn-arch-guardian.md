---
title: Nakamoto-Guardian Journal 🛡️
layout: page
permalink: /Nakamoto-Guardian
---

# Nakamoto-Guardian Journal 🛡️ (ATS v2.0)
...
- **Privacy**: Nakamoto-Guardian AI must redact PII from Travala/Silent.Link web-view hand-offs.
...
## 2026-03-10 - [API Key Case-Sensitivity and Punctuation-Based Injection Bypass]
**Vulnerability:** AI Service API keys (e.g., `SK-PROJ-...`) could bypass redaction if provided in uppercase because the `apiKeyRegex` lacked the case-insensitive flag. Additionally, prompt injection filters could be bypassed by inserting punctuation (e.g., `ignore_previous_instructions`) between words, as the regex only accounted for whitespace.
**Learning:** Security filters must be resilient to common obfuscation techniques used to evade keyword matching. LLMs often treat words joined by punctuation as semantically equivalent to whitespace-separated words, but simple regexes fail if they don't explicitly allow for non-alphanumeric separators.
**Prevention:** Always use the case-insensitive (`i`) flag for API key and secret redaction. When generating regexes for blacklist patterns, allow for any amount of whitespace, punctuation, or underscores (`[\s\W_]*`) between words to prevent simple obfuscation bypasses.

## 2026-03-11 - [Incomplete Redaction due to Underscore Word Boundaries]
